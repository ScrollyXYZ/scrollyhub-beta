<template>
  <div>
    ...
    <div class="mt-3">
      <p v-if="typeChoice == 0">
        The "Onchain image(s)" option means that metadata is stored onchain
        (within the smart contract itself). Use this option if you want to add
        an image URL in the field below (not a metadata file). This is the
        default option for NFTs created through this launchpad.
      </p>

      <p v-if="typeChoice == 1">
        The "Static metadata URL" option means that all the NFTs in the
        collection have the same (static) image and meta data. The metadata is
        stored offchain, usually on IPFS or on a centralized server. Use this
        option if you have a URL of a single metadata
        <strong>file</strong> (all NFTs have the same metadata and image).
      </p>

      <p v-if="typeChoice == 2">
        The "Generative metadata (.json)" option means that all the NFTs in the
        collection have a different image and different meta data. The metadata
        is stored offchain, usually on IPFS or on a centralized server. Use this
        option if you have a metadata
        <strong>folder</strong> URL with multiple metadata files in it, where
      ...
      <p v-if="typeChoice == 3">
        ...
        collection have a different image and different meta data. The metadata
        is stored offchain, usually on IPFS ou on a centralized server. Use this
        option if you have a metadata
        <strong>folder</strong> URL with multiple metadata files in it, where
        these files DO NOT HAVE a .json extension.
      </p>
    </div>
    ...
    <div class="mt-4" v-if="typeChoice > 0">
      ...
      <div id="previewImageHelp" class="form-text">
        This is a preview image for the whole collection, not for individual
        NFTs.
      </div>
    </div>
    <button
