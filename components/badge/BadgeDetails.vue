  ...
  methods: {
    async attach() {
      ...
      try {
        ...
        if (receipt.status === 1) {
          ...
          this.toast(
            "You have successfully attached your badge to a Scroll Canvas profile!",
            {
              type: "success",
              onClick: () =>
                window
    ...
    async checkAttached() {
      if (this.attId && this.profileAddr) {
        ...
        try {
          ...
          console.error(error);
          this.toast.error(
            "An error occurred with the getAttachedBadges() call. Please try again later.",
          );
        }
      }
    },
    ...
    async fetchProfileAddress() {
      ...
      try {
        ...
        const isProfileMinted = await contract.isProfileMinted(
          this.profileAddr,
        );
        if (isProfileMinted) {
